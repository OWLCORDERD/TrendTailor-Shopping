...
import { searchNaverShop } from "./api/coupang.service";
  ...
  async collectClothesForKeyword(keyword: searchKeywordType) {
    ...
    for (const task of searchTasks) {
      ...
      try {
        ...
        if (!result.success) {
          console.error(`검색 요청 실패 [${task.query}]: ${result.err}`);
          continue;
        }
        ...
        console.error(`검색 요청 중 오류 발생 [${task.query}]: ${err}`);
        continue;
      }
    }
